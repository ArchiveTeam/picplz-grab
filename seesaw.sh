#!/bin/bash
#
# Distributed downloading script for Picplz.com.
#
# This will get a task from the tracker and download a Picplz user.
#
...
VERSION=$( grep 'VERSION=' dld-picplz-user.sh | grep -oE "[-0-9.]+" )
...
do
  ...
  then
    ...
    then
      ...
      prefix_dir="${user_id_8:7:1}/${user_id_8:6:2}/${user_id_8:5:3}"
      prefix_file="$prefix_dir/picplz-$user_id_8-"
      bytes=$( ./du-helper.sh -b "data/$prefix_dir/picplz-$user_id_8-"*".warc.gz" )
      bytes_str="{\"user\":${bytes}}"

      picplz_id=$( cat "data/$prefix_dir/picplz-$user_id_8.json" )

      success_str="{\"downloader\":\"${youralias}\",\"item\":\"${itemname}\",\"bytes\":${bytes_str},\"version\":\"${VERSION}\",\"id\":${picplz_id}}"
      ...
      while [ $result -ne 0 ]
      do
        ls -1 "$prefix_file"*".warc.gz" "$prefix_dir/picplz-$user_id_8.json" | \
        rsync -avz \
              --compress-level=9 \
              --progress \
              ${bwlimit} \
              ...
              --remove-source-files \
              --files-from="-" \
              ./ fos.textfiles.com::picplz/${youralias}/
        result=$?
